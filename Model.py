# train_model.py -- Full corrected script (CUDA‑only, A10G‑ready)
...
def setup_ddp(rank:int, world:int):
...
# -------------- Dataset --------------
def get_shards(folder:str):
    if not os.path.isdir(folder):
        raise FileNotFoundError(folder)
    files = glob.glob(os.path.join(folder,"*.pt"))
    if not files:
        raise FileNotFoundError("no .pt files in "+folder)
    if is_main(): print(f"Found {len(files)} shard(s)")
    return files
...
class ChunkedText(Dataset):
    def __init__(self, files:List[str], block:int):
        ...
        if is_main(): print(f"Dataset chunks: {len(self)}")
...
class NiroLM(nn.Module):
    def __init__(self,vocab,block,n_embd,n_heads,n_layer,drop):
        ...
        self.blocks=nn.Sequential(*[Block(n_embd,n_heads,block,drop) for _ in range(n_layer)])
...
def lr_sched(step,base,warm,total):
    if step<warm: return base*step/warm
    if step>total: return base*0.1
...
def dataloader(ds,bs,rank,world,shuffle=True):
    samp=DistributedSampler(ds,num_replicas=world,rank=rank,shuffle=shuffle)
    return DataLoader(ds,batch_size=bs,sampler=samp,pin_memory=True,num_workers=4,drop_last=True)

def save_ckpt(model,opt,epoch,name):
    if is_main():
        torch.save({'epoch':epoch,
                    'model':model.state_dict(),
                    'optim':opt.state_dict()},name)
        print('Saved',name)
# ---------------------------------
...
def main():
    ...
    multi=world>1
    device=setup_ddp(rank,world) if multi else torch.device('cuda')
    if not multi: torch.cuda.set_device(device)
    ...
    tok=spm.SentencePieceProcessor(); tok.load(TOKENIZER_MODEL_PATH)

    model=NiroLM(VOCAB_SIZE,BLOCK_SIZE,N_EMBD,N_HEADS,N_LAYER,DROPOUT).to(device)
    if multi: model=DDP(model,device_ids=[device])
    ...
    for ep in range(1,NUM_EPOCHS+1):
        if multi: train_dl.sampler.set_epoch(ep)
        ...
        for i,(x,y) in enumerate(prog):
            ...
            if (i+1)%GRAD_ACC==0:
                ...
                if gstep%EVAL_STEPS==0 and is_main():
                    val=estimate(model,val_dl,device,EVAL_ITERS)
                    print(f"step {gstep}: val loss {val:.4f}, ppl {math.exp(val):.2f}")
                    save_ckpt(model.module if multi else model,opt,ep,'niro_best.pth')
        if is_main(): save_ckpt(model.module if multi else model,opt,ep,f"niro_ep{ep}.pth")
    if is_main():
